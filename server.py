...
async def read_text_lines(stream: asyncio.StreamReader) -> AsyncGenerator[str, None]:
    while not stream.at_eof():
        ...
        text_line = text_line_bytes.decode(
            "cp1251"
        )  # FIXME Правильно ли выбрана кодировка?
...
async def run_py_script(
    ...
    *,
    ...
    try:
        yield read_text_lines(
            process.stdout
        )  # yield a whole async generator, not a single value
    finally:
        ...
        with suppress(asyncio.TimeoutError):
            result = await asyncio.wait_for(
                process.communicate(), timeout=termination_delay
            )
...
class TelegramBot:
    ...
    @staticmethod
    async def _run_sync_in_executor(method: Callable, *args: object) -> dict[str, Any]:
    ...
    def _encode_multipart_formdata(
        fields: dict[str, Any], files: list[tuple[str, Path, bytes]]
    ) -> tuple[str, bytes]:
    ...
    async def send_message(
        ...
        return await self._run_sync_in_executor(
            self.send_message_sync, chat_id, message, keyboard
        )
    ...
    async def edit_message_reply_markup(
        ...
        return await self._run_sync_in_executor(
            self.edit_message_reply_markup_sync, chat_id, message_id, keyboard
        )
    ...
    async def send_photo(
        self, chat_id: int | str, photo: str | Path, caption: str | None = None
    ) -> dict[str, Any] | None:
    ...
    def send_photo_by_url_sync(
        self, chat_id: int | str, photo_url: str, caption: str | None = None
    ) -> dict[str, Any]:
        method = "sendPhoto"
        params = {
            "chat_id": chat_id,
    ...
    async def send_photo_by_url(
        ...
        return await self._run_sync_in_executor(
            self.send_photo_by_url_sync, chat_id, photo_url, caption
        )

    def send_photo_by_path_sync(
        self, chat_id: int | str, photo_path: Path, caption: str | None = None
    ) -> dict[str, Any]:
        method = "sendPhoto"
    ...
    async def send_photo_by_path(
        self, chat_id: int | str, photo_path: str | Path, caption: str | None = None
    ) -> dict[str, Any]:
        return await self._run_sync_in_executor(
            self.send_photo_by_path_sync, chat_id, photo_path, caption
        )
    ...
    async def run_long_polling(
        ...
        offset = None
        while True:
            updates = await self._run_sync_in_executor(
                self.get_updates_sync, offset, timeout
            )
            if updates is None:
                logger.error(
                    "Не удалось получить обновления, повтор запроса через 5 секунд"
                )
...
async def process_script(
    stdout: AsyncGenerator[str, None], bot: TelegramBot
...
async def main():
    ...
    async for update in bot.run_long_polling():
        ...
        logger.info(
            "Получено обновление от Telegramm: message_id - %s, chat_id - %s",
            message_id,
            chat_id,
        )
        ...
        try:
            ...
            if error.subprocess_exit_code != 0:
                logger.error(
                    "Скрипт завершил работу с кодом ошибки %s",
                    error.subprocess_exit_code,
                )
